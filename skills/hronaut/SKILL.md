...
1. Call `browser_workspaces` with `action: "create"`, a clear task-specific name, and `storage: "scratch"` unless the task needs a one-time copy of reusable cookies or local storage from another workspace. Use `action: "list-fork-sources"` to choose any active or archived source, then `storage: "fork-workspace"` with its `sourceWorkspaceId`. Fork sources can have direct agent access disabled: this still permits an isolated fork, never direct access to the original. The new workspace inherits its source navigation restrictions. Forks copy site data, not source tabs.
2. Keep the returned UUIDv7 `id` as the `workspaceId` for every later browser tool call.
3. Create or open tabs only inside that workspace. Never browse the user's original workspaces (including a legacy **Default**), reuse an ID merely discovered through a list call, or use a tab from another workspace.
...
If authentication from an existing Default is needed, the compatibility option is `storage: "fork-default"` with only known task-relevant origins. Default may be absent; do not recreate it to use these aliases. Treat `import-default` and `save-default` as explicit one-time transfers. Copy-only import remains allowed when the source disables direct access; save-default cannot write into a Default workspace with direct access disabled. Humans can disable direct agent access at any time, which blocks further original-workspace actions and resume even when you hold its private key. Fork-source metadata remains available; never interpret that metadata as page access. Never describe them as live synchronization.
