...
test('keeps empty workspaces visible and opens a tab from each workspace action', async ({
  ...
  try {
    ...
    expect(Math.round(workspaceBounds?.height ?? 0)).toBe(29)
    expect(Math.round(newTabBounds?.height ?? 0)).toBe(28)
    expect(Math.round(createBounds?.height ?? 0)).toBe(28)
    const groupBounds = await workspaceControl.locator('..').boundingBox()
    expect(groupBounds).not.toBeNull()
    expect(workspaceBounds!.y).toBeGreaterThan(groupBounds!.y)
    expect(newTabBounds!.x).toBeGreaterThanOrEqual(workspaceBounds!.x + workspaceBounds!.width)
    expect(newTabBounds!.x + newTabBounds!.width).toBeLessThan(groupBounds!.x + groupBounds!.width)
    expect(newTabBounds!.y + newTabBounds!.height).toBeLessThan(groupBounds!.y + groupBounds!.height)
