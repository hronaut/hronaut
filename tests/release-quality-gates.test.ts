...
describe('release quality gates', () => {
  ...
  it('retains bounded Playwright diagnostics when Docker integration fails', async () => {
    ...
    expect(pullRequestIntegration).toContain('name: Electron integration shard ${{ matrix.shard }}/5')
    expect(pullRequestIntegration).toContain('HRONAUT_INTEGRATION_SHARD: "${{ matrix.shard }}/5"')
    expect(releaseIntegration).toContain('fail-fast: false')
    expect(releaseIntegration).toContain('name: Electron integration shard ${{ matrix.shard }}/5')
    expect(releaseIntegration).toContain('shard: [1, 2, 3, 4, 5]')
    expect(releaseIntegration).toContain('HRONAUT_INTEGRATION_SHARD: "${{ matrix.shard }}/5"')
  ...
  it('publishes a safe reproducible wallet QA quickstart', async () => {
    ...
    expect(fixture).toContain("const method = 'eth_getTransactionReceipt'")
    expect(fixture).toContain("stage: 'provider-selection'")
    expect(fixture).toContain("stage: 'provider-validation'")
    expect(fixture).toContain("stage: 'provider-rejection'")
