...
describe('release quality gates', () => {
  ...
  it('retains bounded Playwright diagnostics when Docker integration fails', async () => {
    ...
    expect(pullRequestIntegration).toContain('name: Electron integration shard ${{ matrix.shard }}/4')
    expect(pullRequestIntegration).toContain('HRONAUT_INTEGRATION_SHARD: "${{ matrix.shard }}/4"')
    expect(releaseIntegration).toContain('fail-fast: false')
    expect(releaseIntegration).toContain('name: Electron integration shard ${{ matrix.shard }}/4')
    expect(releaseIntegration).toContain('shard: [1, 2, 3, 4]')
    expect(releaseIntegration).toContain('HRONAUT_INTEGRATION_SHARD: "${{ matrix.shard }}/4"')
