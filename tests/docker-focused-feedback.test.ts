...
describe('focused Docker integration feedback', () => {
  it('builds only the dependency stage and runs the live checkout', async () => {
    ...
    expect(dockerfile).toContain('COPY --from=install-inputs /install/package.json /install/package-lock.json ./')
    expect(launcher).toContain("installInputSource: readFileSync('scripts/docker-install-inputs.ts', 'utf8')")
    expect(dockerfile).toContain('COPY scripts/docker-dependency-cache-key.ts ./scripts/')
    ...
    expect(launcher).toContain('const cacheKey = dependencyCacheKey({')
    expect(launcher).toContain("packageLockSource: readFileSync('package-lock.json', 'utf8')")
    expect(launcher).toContain("cacheKeySource: readFileSync('scripts/docker-dependency-cache-key.ts', 'utf8')")
    expect(launcher).toContain("readFileSync('Dockerfile.test', 'utf8')")
    expect(launcher).toContain('HRONAUT_DEPENDENCY_CACHE_KEY')
    expect(launcher).toContain("['volume', 'create', volumeName]")
    expect(launcher).toContain("'--project-name', `hronaut-focused-${cacheKey}`")
    ...
    expect(launcher).toContain("['out', 'test-results', 'playwright-report', '.cache']")
  ...
  it('distributes hosted Electron shards across isolated runners', async () => {
    ...
    expect(workflow).toContain('name: Electron integration shard ${{ matrix.shard }}/5')
    expect(workflow).toContain('shard: [1, 2, 3, 4, 5]')
    expect(workflow).toContain('HRONAUT_INTEGRATION_SHARD: "${{ matrix.shard }}/5"')
