...
describe('focused Docker integration feedback', () => {
  it('builds only the dependency stage and runs the live checkout', async () => {
    ...
    expect(launcher).toContain("['scripts/docker-dependency-cache-key.ts', 'package-lock.json']")
    expect(launcher).toContain('.update(dependencyLockHash.stdout.trim())')
    expect(launcher).toContain(".update(readFileSync('scripts/docker-dependency-cache-key.ts'))")
