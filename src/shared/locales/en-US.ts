export const enUS = {
  ...
  settings: {
    ...
    mcp: { heading: 'MCP security', description: 'Control which local applications can connect to this browser profile.', require: 'Require MCP authentication', requireDescription: 'Require the owner-only per-profile bearer token for MCP and health requests.', toolSet: 'MCP tool set', toolSetDescription: 'Choose the task-focused catalog for new MCP connections.', toolSetEssentials: 'Browser Essentials', toolSetQa: 'Web QA', toolSetComplete: 'Complete', toolSetReconnect: 'Connected MCP clients keep their current catalog. Reconnect them to apply this change.', port: 'MCP server port', portDescription: 'Move the local listener without restarting Hronaut. Connected clients must use the new endpoint.', moving: 'Moving…', applyPort: 'Apply port', tokenHelp: 'The token is generated once per profile and never displayed on Hronaut Home.', warning: 'Authentication is off. Any process on this computer can control logged-in tabs and attach local files.' },
