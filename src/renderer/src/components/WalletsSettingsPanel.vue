...
<template>
  <div class="settings-content wallet-settings">
    ...
    <form v-if="controller.status.value.managedWallets === 'passphrase-setup-required'" class="wallet-card" @submit.prevent="submitPassphrase('setup')">
      ...
      <UiButton appearance="application" variant="primary" class="primary-button" type="submit" :disabled="controller.busy.value">{{ t('wallets.createEncryptedVault') }}</UiButton>
    </form>
    <form v-else-if="controller.status.value.managedWallets === 'locked' && vaultUsesPassphrase" class="wallet-card wallet-unlock-card" @submit.prevent="submitPassphrase('unlock')">
      ...
      <UiButton appearance="application" variant="primary" class="primary-button" type="submit" :disabled="controller.busy.value">{{ t('wallets.unlock') }}</UiButton>
    </form>
    <section v-else-if="controller.status.value.managedWallets === 'locked'" class="wallet-card wallet-unlock-card">
      ...
      <UiButton appearance="application" variant="primary" class="primary-button" type="button" :disabled="controller.busy.value" @click="unlockSystemStorage">{{ t('wallets.unlockSystemStorage') }}</UiButton>
    </section>
    ...
    <section class="wallet-card">
      ...
      <form v-if="!preparedImport" class="wallet-form" @submit.prevent="submitOnboarding">
        ...
        <UiButton appearance="application" variant="primary" class="primary-button wallet-submit-button" type="submit" :disabled="onboardingLocked || controller.busy.value || !onboardingNetworkValid || (mode === 'watch' ? !controller.status.value.watchOnlyAvailable : controller.status.value.managedWallets !== 'ready')">{{ onboardingSubmitLabel }}</UiButton>
      </form>
      <div v-if="preparedImport" ref="preparedImportReview" class="wallet-import-confirm" role="status" :aria-busy="cancellingImport || undefined">
        ...
        <div class="wallet-actions">
          <UiButton appearance="application" variant="primary" ref="addImportedWalletButton" class="primary-button" type="button" :disabled="controller.busy.value || cancellingImport" @click="confirmPreparedImport">{{ t('wallets.addEncryptedWallet') }}</UiButton>
          <UiButton appearance="application" class="secondary-button" type="button" :disabled="controller.busy.value || cancellingImport" @click="cancelPreparedImport"><IconProgress v-if="cancellingImport" class="state-spinner" aria-hidden="true" />{{ t('wallets.cancel') }}</UiButton>
        </div>
      </div>
    </section>
    ...
    <section ref="configuredWalletsSection" class="wallet-card">
      ...
      <div v-if="controller.status.value.managedWallets === 'ready' && hasSigningWallet" class="wallet-vault-control">
        ...
        <UiButton appearance="application" class="secondary-button" type="button" :disabled="controller.busy.value" @click="controller.lock">{{ t('wallets.lockSigningKeys') }}</UiButton>
      </div>
      ...
      <template v-if="selectedWallet">
        ...
        <form v-if="renamingWallet" class="wallet-rename-form" @submit.prevent="saveWalletName"><label>{{ t('wallets.walletName') }} <input v-model="renameDraft" maxlength="128" required :disabled="controller.busy.value" @keydown.esc="cancelWalletRename"></label><div class="wallet-actions"><UiButton appearance="application" variant="primary" class="primary-button" type="submit" :disabled="controller.busy.value || !renameDraft.trim()">{{ t('wallets.saveName') }}</UiButton><UiButton appearance="application" class="secondary-button" type="button" :disabled="controller.busy.value" @click="cancelWalletRename">{{ t('wallets.cancel') }}</UiButton></div></form>
        <form v-if="editingRpc" class="wallet-rpc-form" @submit.prevent="saveWalletRpc"><label>{{ configuredRpcLabel }} <input v-model="rpcDraft" type="url" required :disabled="controller.busy.value" :aria-invalid="configuredRpcValid ? undefined : 'true'" @keydown.esc="cancelWalletRpcEdit"></label><p>{{ t('wallets.rpcChangeWarning') }}</p><div class="wallet-actions"><UiButton appearance="application" variant="primary" class="primary-button" type="submit" :disabled="controller.busy.value || !configuredRpcValid || rpcDraft.trim() === selectedWallet.network.rpcUrl">{{ t('wallets.saveRpc') }}</UiButton><UiButton appearance="application" class="secondary-button" type="button" :disabled="controller.busy.value" @click="cancelWalletRpcEdit">{{ t('wallets.cancel') }}</UiButton></div></form>
        <div class="wallet-actions"><UiButton appearance="application" class="secondary-button" type="button" :disabled="controller.busy.value" @click="attachWorkspaces">{{ t('wallets.saveWorkspaceAccess') }}</UiButton><UiButton appearance="application" v-if="!renamingWallet" class="secondary-button" type="button" :disabled="controller.busy.value" @click="renameWallet">{{ t('wallets.rename') }}</UiButton><UiButton appearance="application" v-if="!editingRpc" class="secondary-button" type="button" :disabled="controller.busy.value" @click="editWalletRpc">{{ t('wallets.changeRpc') }}</UiButton><UiButton appearance="application" variant="danger" class="danger-button" type="button" :disabled="controller.busy.value" @click="removeWallet">{{ t('wallets.remove') }}</UiButton></div>
