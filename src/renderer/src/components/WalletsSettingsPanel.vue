    ...
    <section class="wallet-card">
      ...
      <template v-if="selectedWallet">
        ...
        <section class="wallet-access-panel wallet-configured-access" aria-labelledby="wallet-configured-access-heading">
          ...
          <div class="wallet-scope-options" role="radiogroup" :aria-label="t('wallets.workspaceAccessHeading')">
            <label class="wallet-choice-card"><input v-model="configuredWorkspaceScope" type="radio" value="selected" :aria-label="t('wallets.selectedWorkspaces')"><span><strong>{{ t('wallets.selectedWorkspaces') }}</strong><small>{{ t('wallets.selectedWorkspacesDescription') }}</small></span></label>
            <label class="wallet-choice-card"><input v-model="configuredWorkspaceScope" type="radio" value="all" :aria-label="t('wallets.anyWorkspace')"><span><strong>{{ t('wallets.anyWorkspace') }}</strong><small>{{ t('wallets.anyWorkspaceDescription') }}</small></span></label>
          </div>
          <fieldset :disabled="configuredWorkspaceScope === 'all'"><legend>{{ t('wallets.chooseWorkspaces') }}</legend><label v-for="workspace in workspaces" :key="workspace.id"><input v-model="configuredWorkspaceIds" type="checkbox" :value="workspace.id"> {{ workspace.name }}</label><p v-if="workspaces.length === 0">{{ t('wallets.noWorkspaces') }}</p></fieldset>
        ...
        <div class="wallet-actions"><button class="secondary-button" type="button" @click="attachWorkspaces">{{ t('wallets.saveWorkspaceAccess') }}</button><button v-if="!renamingWallet" class="secondary-button" type="button" @click="renameWallet">{{ t('wallets.rename') }}</button><button v-if="!editingRpc" class="secondary-button" type="button" @click="editWalletRpc">{{ t('wallets.changeRpc') }}</button><button class="danger-button" type="button" @click="removeWallet">{{ t('wallets.remove') }}</button></div>
