...
<template>
  <section v-if="open" class="network-content-search" :aria-label="t('network.searchContent')">
    <form @submit.prevent="emit('search')">
      ...
      <button type="submit" class="primary" :disabled="state === 'searching' || !query.trim()"><IconProgress v-if="state === 'searching'" class="state-spinner" aria-hidden="true" /><IconSearch v-else aria-hidden="true" />{{ state === 'searching' ? t('network.contentSearch.searching') : t('network.contentSearch.search') }}</button>
      <button type="button" :aria-label="t('network.contentSearch.close')" @click="emit('close')"><IconClose aria-hidden="true" /></button>
    </form>
    ...
    <template v-if="result">
      ...
      <div v-if="result.matches.length" class="network-content-search-results">
        <button v-for="(match, index) in result.matches" :key="`${match.requestId}:${match.field}:${match.label}:${index}`" type="button" :aria-label="t('network.contentSearch.inspect', { number: localNumber(index + 1), label: match.label })" @click="emit('select', match)">
