    ...
    <div class="page-tools-content">
      <section aria-labelledby="page-tools-inspect-title">
        ...
        <div class="page-tools-grid">
          <UiButton appearance="application" type="button" :aria-label="activeWebUrl ? t('runtime.tabs.siteStorage', { host: hostname }) : t('runtime.tabs.siteStorageUnavailable')" :disabled="!activeWebUrl" @click="actions.toggleSiteStorage">
          ...
          <UiButton appearance="application" :class="{ complete: Boolean(activeEmulation?.viewport) }" type="button" :aria-label="t('runtime.address.responsive', { status: labels.responsive })" @click="actions.toggleResponsivePreview">
            <IconDevices aria-hidden="true" /><span><strong>{{ t('shell.pageTools.responsive') }}</strong><small>{{ labels.responsive }}</small></span>
          </UiButton>
          <UiButton appearance="application" :class="{ complete: environmentOverrideCount > 0, error: environmentState === 'error', running: environmentState === 'applying' }" type="button" :aria-label="t('runtime.address.environment', { status: labels.environment })" :disabled="environmentState === 'applying'" @click="actions.toggleEnvironment">
          ...
          <UiButton appearance="application" type="button" :aria-label="t('shell.pageTools.openConsole')" @click="actions.toggleConsole"><IconTerminal aria-hidden="true" /><span><strong>{{ t('panels.console') }}</strong><small>{{ t('shell.pageTools.consoleDescription') }}</small></span></UiButton>
          <UiButton appearance="application" type="button" :aria-label="t('shell.pageTools.openNetwork')" @click="actions.toggleNetwork"><IconNetworkCheck aria-hidden="true" /><span><strong>{{ t('panels.network') }}</strong><small>{{ t('shell.pageTools.networkDescription') }}</small></span></UiButton>
          <UiButton appearance="application" :class="{ warning: networkRouteCount > 0 }" type="button" :aria-label="t('runtime.address.conditions', { status: networkRouteCount ? t('network.conditions.active', { count: localNumber(networkRouteCount) }, networkRouteCount) : t('runtime.address.noneActive') })" @click="actions.openRequestConditions">
      ...
      <section aria-labelledby="page-tools-diagnose-title">
        ...
        <div class="page-tools-grid">
          <UiButton appearance="application" :class="{ warning: inspectorIssueCount > 0 }" type="button" :aria-label="t('issues.toolAria', { status: labels.inspectorIssues })" @click="toggleInspectorIssues"><IconWarning aria-hidden="true" /><span><strong>{{ t('panels.issues') }}</strong><small>{{ labels.inspectorIssues }}</small></span></UiButton>
          <UiButton appearance="application" :class="{ complete: securityReport?.state === 'secure', warning: securityReport?.state === 'insecure' || securityReport?.state === 'insecure-broken', error: securityReportState === 'error', running: securityReportState === 'loading' }" type="button" :aria-label="t('securityReport.toolAria', { status: labels.security })" :disabled="securityReportState === 'loading'" @click="toggleSecurityReport">
          ...
          <UiButton appearance="application" :class="{ running: activeTab?.reproRecording?.active }" type="button" :aria-label="t('repro.toolAria', { status: labels.repro })" @click="toggleReproRecorder"><IconRecord aria-hidden="true" /><span><strong>{{ t('panels.reproRecorder') }}</strong><small>{{ labels.repro }}</small></span></UiButton>
          <UiButton appearance="application" :class="{ running: activeTab?.domChangesRecording?.active }" type="button" :aria-label="t('domChanges.toolAria', { status: labels.domChanges })" @click="toggleDomChanges"><IconAccountTree aria-hidden="true" /><span><strong>{{ t('panels.domChanges') }}</strong><small>{{ labels.domChanges }}</small></span></UiButton>
          <UiButton appearance="application" :class="{ complete: visualCompareReport?.status === 'compared' && visualCompareReport.identical, warning: visualCompareReport?.status === 'compared' && !visualCompareReport.identical, error: visualCompareState === 'error', running: visualCompareState === 'loading' }" type="button" :aria-label="t('visualCompare.toolAria', { status: labels.visualCompare })" :disabled="visualCompareState === 'loading'" @click="toggleVisualCompare">
      ...
      <section aria-labelledby="page-tools-export-title">
        ...
        <div class="page-tools-grid">
          ...
          <UiButton appearance="application" type="button" :aria-label="labels.pdfExport" :disabled="pdfState === 'saving'" @click="closeAndRun(actions.savePdf)">
          ...
          <UiButton appearance="application" type="button" :aria-label="credentialCount ? t('shell.pageTools.fillPassword') : t('shell.pageTools.noPassword')" :disabled="!credentialStorageAvailable || !credentialCount" @click="closeAndRun(actions.fillSavedPassword)">
