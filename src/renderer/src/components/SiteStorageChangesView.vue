  ...
  <section class="storage-changes-view" :aria-busy="state === 'loading'">
    ...
    <template v-else-if="report">
      <div v-if="report.status === 'empty'" class="site-storage-empty storage-changes-empty">
        ...
        <UiButton appearance="application" variant="primary" class="primary" type="button" @click="emit('manage', 'baseline')"><IconDifference aria-hidden="true" /> {{ t('siteStorage.trackedChanges.setBaseline') }}</UiButton>
      </div>
      <template v-else>
        ...
        <div v-if="report.status === 'compared' && report.changes.length" class="storage-changes-list">
          <UiButton appearance="application" v-for="(change, index) in report.changes" :key="`${change.kind}-${change.key}-${change.domain ?? ''}-${change.path ?? ''}-${index}`" type="button" class="storage-change" :class="change.type" @click="emit('inspect', change)">
        ...
        <footer class="storage-changes-footer">
          ...
          <div><UiButton appearance="application" type="button" @click="emit('manage', 'clear')"><IconDelete aria-hidden="true" /> {{ t('siteStorage.trackedChanges.clear') }}</UiButton><UiButton appearance="application" type="button" @click="emit('manage', 'baseline')"><IconRefresh aria-hidden="true" /> {{ t('siteStorage.trackedChanges.newBaseline') }}</UiButton><UiButton appearance="application" v-if="report.status === 'compared'" type="button" @click="emit('copy')"><IconCheck v-if="copied" aria-hidden="true" /><IconCopy v-else aria-hidden="true" /> {{ copied ? t('siteStorage.copied') : t('siteStorage.trackedChanges.copy') }}</UiButton><UiButton appearance="application" variant="primary" class="primary" type="button" :disabled="state === 'loading'" @click="emit('manage', 'compare')"><IconDifference aria-hidden="true" /> {{ t('siteStorage.trackedChanges.compare') }}</UiButton></div>
        </footer>
      </template>
    </template>
  </section>
