  ...
  <section v-if="open" class="site-storage-panel" data-shell-docked-panel role="dialog" aria-modal="false" aria-labelledby="site-storage-title">
    <header>
      ...
      <div class="site-storage-header-actions">
        ...
        <UiButton native type="button" :disabled="siteStorageUsageOpen ? siteStorageUsageState === 'loading' : siteStoragePwaOpen ? siteStoragePwaState === 'loading' : siteStorageIndexedDbOpen ? siteStorageIndexedDbState === 'loading' : siteStorageChangesOpen ? siteStorageChangesState === 'loading' : siteStorageState === 'loading' || siteStorageState === 'saving'" @click="refreshActiveSiteStorageView"><IconRefresh aria-hidden="true" /> {{ t('siteStorage.refresh') }}</UiButton>
        <UiButton native class="panel-close" type="button" :aria-label="t('siteStorage.close')" @click="open = false"><IconClose aria-hidden="true" /></UiButton>
      </div>
    </header>
    <nav class="site-storage-kinds" :aria-label="t('siteStorage.typeAria')">
      <UiButton native type="button" :class="{ active: siteStorageUsageOpen }" :aria-pressed="siteStorageUsageOpen" :disabled="siteStorageState === 'saving'" @click="selectSiteStorageUsage"><IconPieChart aria-hidden="true" /> {{ t('siteStorage.overview') }}</UiButton>
      <UiButton native type="button" :class="{ active: !siteStorageUsageOpen && !siteStorageChangesOpen && !siteStorageIndexedDbOpen && !siteStoragePwaOpen && siteStorageKind === 'local-storage' }" :aria-pressed="!siteStorageUsageOpen && !siteStorageChangesOpen && !siteStorageIndexedDbOpen && !siteStoragePwaOpen && siteStorageKind === 'local-storage'" :disabled="siteStorageState === 'saving'" @click="selectSiteStorageKind('local-storage')"><IconDatabase aria-hidden="true" /> {{ t('siteStorage.local') }}</UiButton>
      <UiButton native type="button" :class="{ active: !siteStorageUsageOpen && !siteStorageChangesOpen && !siteStorageIndexedDbOpen && !siteStoragePwaOpen && siteStorageKind === 'session-storage' }" :aria-pressed="!siteStorageUsageOpen && !siteStorageChangesOpen && !siteStorageIndexedDbOpen && !siteStoragePwaOpen && siteStorageKind === 'session-storage'" :disabled="siteStorageState === 'saving'" @click="selectSiteStorageKind('session-storage')"><IconDatabase aria-hidden="true" /> {{ t('siteStorage.session') }}</UiButton>
      <UiButton native type="button" :class="{ active: !siteStorageUsageOpen && !siteStorageChangesOpen && !siteStorageIndexedDbOpen && !siteStoragePwaOpen && siteStorageKind === 'cookies' }" :aria-pressed="!siteStorageUsageOpen && !siteStorageChangesOpen && !siteStorageIndexedDbOpen && !siteStoragePwaOpen && siteStorageKind === 'cookies'" :disabled="siteStorageState === 'saving'" @click="selectSiteStorageKind('cookies')"><IconCookie aria-hidden="true" /> {{ t('siteStorage.cookies') }}</UiButton>
      <UiButton native type="button" :class="{ active: siteStorageIndexedDbOpen }" :aria-pressed="siteStorageIndexedDbOpen" :disabled="siteStorageState === 'saving'" @click="selectSiteStorageIndexedDb"><IconDatabase aria-hidden="true" /> {{ t('runtime.storage.indexedDb') }}</UiButton>
      <UiButton native type="button" :class="{ active: siteStoragePwaOpen }" :aria-pressed="siteStoragePwaOpen" :disabled="siteStorageState === 'saving'" @click="selectSiteStoragePwa"><IconOffline aria-hidden="true" /> {{ t('siteStorage.offline') }}</UiButton>
      <UiButton native type="button" :class="{ active: siteStorageChangesOpen }" :aria-pressed="siteStorageChangesOpen" :disabled="siteStorageState === 'saving'" @click="selectSiteStorageChanges"><IconDifference aria-hidden="true" /> {{ t('siteStorage.changes') }}</UiButton>
    </nav>
    ...
    <template v-else-if="!siteStorageChangesOpen && !siteStorageIndexedDbOpen && !siteStoragePwaOpen">
      <div class="site-storage-tools">
        ...
        <UiButton native class="site-storage-clear" type="button" :disabled="!siteStorageResult?.itemCount || siteStorageState === 'saving'" @click="clearSiteStorageKind"><IconDelete aria-hidden="true" /> {{ t('siteStorage.clearKind', { kind: siteStorageKindLabel.toLocaleLowerCase(locale) }) }}</UiButton>
      </div>
      <form class="site-storage-editor" @submit.prevent="saveSiteStorageItem">
        ...
        <UiButton native type="submit" :disabled="!siteStorageKey.trim() || siteStorageState === 'saving'">{{ siteStorageResult?.items.some((item) => item.key === siteStorageKey) ? t('siteStorage.update') : t('siteStorage.add') }}</UiButton>
      </form>
      <div class="site-storage-list" :aria-busy="siteStorageState === 'loading'">
        ...
        <template v-else>
          <article v-for="(item, index) in filteredSiteStorageItems" :key="`${item.key}-${item.domain ?? ''}-${item.path ?? ''}-${index}`" class="site-storage-item" :class="{ protected: item.protected }">
            <UiButton native class="site-storage-item-main" type="button" :disabled="item.protected || siteStorageState === 'saving'" :title="item.protected ? t('siteStorage.protectedTitle') : t('siteStorage.editTitle')" @click="editSiteStorageItem(item)">
