...
interface PersistedMcpCapabilityProfile extends McpCapabilityProfile {
...
function publicProfile(profile: PersistedMcpCapabilityProfile): McpCapabilityProfile {
...
export class McpCapabilityProfileStore {
  ...
  async create(input: McpCapabilityProfileInput): Promise<{ profile: McpCapabilityProfile; credential: string }> {
    return this.queueMutation(async () => {
      ...
      return { profile: publicProfile(profile), credential }
    })
  }
  ...
  async revoke(id: string): Promise<McpCapabilityProfile> {
    return this.queueMutation(async () => {
      const existing = this.requireProfile(id)
      if (existing.revokedAt) return publicProfile(existing)
  ...
  authenticate(credential: string): McpCapabilityGrant | null {
    ...
    if (!match || !this.profileActive(match)) return null
  ...
  requireActiveGrant(grant: McpCapabilityGrant): McpCapabilityProfile {
    ...
    if (!profile || profile.revision !== grant.revision || profile.credentialId !== grant.credentialId
      || !this.profileActive(profile)) throw new McpCapabilityAuthorizationError()
    return publicProfile(profile)
  }
  ...
  authorize(grant: McpCapabilityGrant, request: McpCapabilityRequest): McpCapabilityProfile {
    ...
    if (!profile || !this.requestAllowed(profile, request)) throw new McpCapabilityAuthorizationError()
    return publicProfile(profile)
  }
  ...
  authorizeActiveDispatch(grant: McpCapabilityGrant, request: McpCapabilityRequest): McpCapabilityProfile {
    ...
    if (!profile || profile.revision !== grant.revision || profile.credentialId !== grant.credentialId
      || profile.revokedAt
      || (profile.expiresAt !== undefined && Date.parse(profile.expiresAt) <= this.now().getTime())
      || (profile.maxUses !== undefined && profile.useCount > profile.maxUses)
      || !this.requestAllowed(profile, request)) throw new McpCapabilityAuthorizationError()
    return publicProfile(profile)
  }
  ...
  async authorizeAndConsume(grant: McpCapabilityGrant, request: McpCapabilityRequest): Promise<McpCapabilityProfile> {
    return this.queueMutation(async () => {
      const profile = this.profiles.get(grant.profileId)
      if (!profile || profile.revision !== grant.revision || profile.credentialId !== grant.credentialId
        || !this.profileActive(profile) || !this.requestAllowed(profile, request)) {
        throw new McpCapabilityAuthorizationError()
      }
      if (profile.maxUses === undefined) return publicProfile(profile)
      const nextProfile: PersistedMcpCapabilityProfile = {
        ...profile,
        useCount: profile.useCount + 1,
        updatedAt: this.now().toISOString()
      }
      const next = new Map(this.profiles)
      next.set(profile.id, nextProfile)
      await this.persist(next.values())
      this.replaceProfiles(next)
  ...
  private profileActive(profile: PersistedMcpCapabilityProfile): boolean {
