...
Requests advance through `draft → validated → simulated → policy-decision → awaiting-human/approved → signing → submitted → confirmed`, or a terminal rejected, expired, cancelled, or failed state. Signing checks the exact normalized request hash again immediately before key use. Durable request IDs and idempotency keys prevent duplicate submission.
