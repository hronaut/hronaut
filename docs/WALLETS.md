...
The default policy is deny/ask. Mainnet signing and sending always require a person. Automatic approval is available for allowlisted EVM testnets whose chain ID is independently attested by the RPC adapter, or a known local-chain ID paired with a loopback RPC URL. Solana and TRON public testnets currently require human approval until their adapters can bind an independently verified genesis/network fingerprint into the request. A user-supplied `testnet` label is never trusted by itself: unknown, mislabeled, public-hosted local, and mainnet networks always return to human approval. Eligible policies can restrict wallet/workspace, network, origin, destination/contract/program, method/instruction, per-transaction native amount, session/daily spend, fee, expiry, operation count, and successful simulation. Hronaut revalidates the exact selected policy immediately before automatic approval; removing or tightening it while a request is in flight prevents that stale request from auto-signing. Unknown or undecodable actions, unlimited approvals, new contracts/programs, blind/raw messages, permission changes, recovery export, and vault changes always require a person.
