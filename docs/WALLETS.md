...
Requests advance through `draft → validated → simulated → policy-decision → awaiting-human/approved → signing → submitted → confirmed`, or a terminal rejected, expired, cancelled, or failed state. A simulation, audit, or state-preparation error moves its request to `failed` immediately, so trusted chrome cannot approve a request whose caller already received an error. An untouched request expires automatically at its displayed deadline, rejects the waiting caller, and clears any retained in-memory message bytes; no approval click or application restart is required. Signing checks the exact normalized request hash again immediately before key use. Durable request IDs and idempotency keys prevent duplicate submission.
