...
The workspace editor offers **Allow direct agent access**. Disabling it blocks direct agent actions and resume access to that original workspace. The normal agent workspace list still shows a metadata-only entry marked `forkOnly`, and the fork-source catalog also lists it. Agents can copy its cookies and local storage into an independent fork. This setting does not prevent cloning. Forks inherit site restrictions and start with one blank tab; they do not copy open tabs.
