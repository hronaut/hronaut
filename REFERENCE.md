...
Open **Settings → Workspaces & data** to review active and archived browser workspaces, manage one workspace, or copy selected cookies and local storage between any two workspaces. Moving data requires both workspaces to be archived so open pages cannot change the source during cleanup. The secondary **Global history & legacy data** section shows local history-page and visit counts, legacy Default-profile cookie count, and HTTP cache size. Select browsing history, cookies and site data, cached images and files, or any combination once, then clear one legacy website or all matching data. The Settings-owned website list never changes its target when another browser tab gains focus. Cookies and site data are off by default because removing them can sign you out; history and cache are selected by default.
...
Use **Archive Workspace** from that context menu when an investigation should leave the active tab strip without being lost. Archived workspaces retain their browser profile and reopen their ordered URLs and pinned state under the same stable `workspaceId`. The legacy Default workspace can also be renamed, restricted, archived, or deleted. Permanently closing or deleting a workspace deletes its browser data; it does not recreate Default. `browser_saved_workspaces` lists only archives authorized for the current MCP connection, and a reconnect must resume its own archive with the private key returned at creation or save time before opening or deleting it.

The legacy base-profile workspace, originally named Default, remains the owner of its existing Chromium data until deleted. Its name is not reserved and no new Default is created automatically. Every newly created workspace receives its own persistent Electron partition for cookies, local storage, cache, service workers, and related website state. On `browser_workspaces` `create`, an MCP client chooses `storage: "scratch"` (the default) for clean isolated site storage or `storage: "fork-workspace"` with an explicit `sourceWorkspaceId` from `list-fork-sources` to copy site data from any active or archived workspace. A fork receives its own identity, partition and private resume key; source tabs are not cloned and an archived source remains archived. The clone permits direct agent access and inherits the source navigation restrictions, including when the source disables direct agent access. Only the human can change direct agent access or navigation policy; a valid resume key never overrides disabled access. The compatibility option `storage: "fork-default"` performs a one-time copy of reusable cookies and local storage from Default, optionally limited with task-relevant `origins` it already knows. Omitting `origins` copies all available cookies and known local storage from Default; MCP deliberately does not expose Default's origin inventory. Later, `import-default` refreshes selected state from Default and `save-default` optionally merges selected workspace state back into Default. Default aliases fail when Default is absent. Copy-only `import-default` remains allowed when Default disables direct agent access, while `save-default` requires an active legacy Default workspace with direct agent access enabled. Use the human transfer controls for archived destinations. These are explicit one-time copies, not a live link or synchronization, and agents remain forbidden from browsing Default directly. `list-origins` shows only the agent workspace and is useful before a selective `save-default`. Hronaut rejects overlapping storage copies and prevents a workspace from being renamed, archived, or deleted while its profile is being copied. Bookmarks, visit history, download records, and Hronaut's remembered permission decisions remain application-wide.
...
An `approve-action` request must include one `review` object for a mutating workspace tool: `toolName`, proposed `arguments`, `reversibility`, and either a bounded `description` or `visible-browser-only` representation, with an optional bounded `expectedPostcondition`. Hronaut validates and normalizes the proposed arguments with the target tool schema, then retains only a domain-separated SHA-256 artifact hash and bounded review metadata. Raw selectors, entered values, scripts, credentials, and other proposed arguments are not written to human-decision history. The visible review identifies the workspace, capability profile, current safe origin and tab where available, action class, reversibility, state generations, deadline, exact artifact hash, and its bounded receipt timeline.
...
Open **Edit workspace → Human decisions** to refresh decisions, acknowledge receipt, reject or cancel a proposed action, or complete a review. Completion requires an explicit checkbox and a fresh workspace continuity review above the panel, checked again after saving. Agents cannot acknowledge, reject, or complete human decisions through MCP. Pending decisions block consequential page tools while inspection remains available. An approved consequential review also blocks every unrelated mutation. Its `id` and current `revision` can be supplied once to the exact normalized tool call; a changed artifact, navigation, human input, workspace, scope, session, expiry, or continuity state invalidates it.

Before that exact tool dispatch, Hronaut durably records `ATTEMPTED`. A matching postcondition changes it to `VERIFIED`; an error or ambiguous result changes it to `UNKNOWN`; a successful action without an authoritative postcondition remains `ATTEMPTED`. Restart converts an unresolved attempt to `UNKNOWN` and expires proposed or approved handles, so reconnecting cannot replay an old approval. Acknowledgement, rejection, cancellation, expiry, and completion never replay actions, unpause agents, erase an unknown outcome, or automatically clear the separate continuity guard. Notification delivery describes dispatch to local attention controls, not proof that a person saw or understood it.
