...
- **Cyberpunk Turbo** for clear reading text, cyan highlights, magenta controls and yellow tab indicators, with a matching Home page.
...
**Interface size** scales Hronaut's tabs, toolbar, Settings, and diagnostic panels independently from website page zoom. New profiles use Compact 100%; choose Comfortable 110% or Large 125% at any time. Detached Hronaut panels follow the same setting, while every website retains its own zoom and viewport. The same dialog includes separate **Search engine**, **Downloads**, **Performance**, **MCP security**, **Workspaces & data**, **Site permissions**, **Passwords**, and **Updates** sections.
...
Obsolete base-profile browser data, workspaces without isolated storage, and tab snapshots older than version 3 are deleted at startup. Supported isolated workspaces retain their identities and storage. No legacy profile migration or Default-workspace aliases are available.
...
Workspace authorization and write ownership are separate. Creating a workspace gives that exact MCP transport a five-minute, renewable, process-local `exclusive-write` lease; resume claims it when available and otherwise grants only shared inspection while reporting the existing lease as busy. Read-only tools remain shareable. A second authorized transport receives `BUSY` with `effects: "none"` before a conflicting mutation is dispatched; it receives no holder identity or foreign lease generation. `browser_workspaces` supports `ownership-status`, `claim-ownership`, and `release-ownership` for explicit inspection and handoff. Its normal `list`, `create`, and `resume` responses include a privacy-safe `writeLease` status.

Disconnect, explicit release, MCP pause, authentication rotation, server stop, or expiry revokes the lease. A command admitted under an older generation must still hold that exact generation immediately before dispatch; it otherwise returns `LEASE_LOST` and cannot silently regain authority after another transport claims the workspace. After pause or takeover, inspect the visible workspace, claim ownership again, and revalidate the tab, origin, account, navigation generation, and intended target. A lease serializes the whole workspace, including separate tabs and shared-origin state; it deliberately does not promise concurrent writes within one workspace or coordination between separate Hronaut processes.
...
**Transfer browser data** lets people select source and destination workspaces. Copy merges cookies and local storage, replacing matching destination entries while retaining unrelated entries. Move verifies the destination before removing the transferred source entries. Both workspaces must be archived for Move so their pages cannot change the data during cleanup. Profiles with background service-worker data cannot be moved through this operation; use Copy instead. These transfers do not include IndexedDB, history, passwords, cache, or downloaded files. Incomplete cleanup is reported explicitly and the verified destination copy is retained.
...
## Collapse the workspace rail when idle

With tabs on the left, choose **Collapse tab rail when not in use** below Home to keep a compact icon rail beside the page. Hover deliberately to reveal tab titles, or use keyboard focus to reveal them immediately. Brief pointer crossings do not open the rail, and a short grace period allows you to leave and return without it closing. It stays open while either the pointer or keyboard focus is inside, and holds its layout through clicks and tab drags.

Home, the pin control, workspace headings, and tabs retain their vertical positions during reveal. Scrolling to other tabs remains in place when the rail changes width; selecting a tab or navigating by keyboard still brings that target into view. Choose **Keep tab rail expanded** to save the expanded preference. Narrow windows temporarily use the compact rail to preserve page space, without changing that saved preference. Drag the rail divider to resize it, or focus the divider and use the arrow keys. Reduced-motion preferences disable the collapse animation.

## Pin important tabs

Right-click a website tab and choose **Pin Tab**, or use the pin control in open-tab search. Pinned tabs stay compact at the left of regular website tabs. Drag tabs along the strip to arrange them; pinned tabs can be reordered among pinned tabs and regular tabs among regular tabs, preserving the compact left boundary. The right-click menu also offers **Move Tab Left/Right** in the horizontal strip or **Move Tab Up/Down** in the vertical rail for an accessible non-drag alternative. **Close Other Tabs**, **Close Tabs to the Right** (horizontal), **Close Tabs Below** (vertical), and **Close Duplicate Tabs** affect only regular tabs in that tab's own workspace; unrelated workspaces and pinned tabs remain intact. Custom order and pinned state are stored with the profile and restored after an application restart, while closed-tab recovery keeps the pin state during the current launch. Unpin from the same native menu or search panel. Home remains separate application navigation and cannot be pinned or reordered.
...
Website downloads appear in the toolbar with live byte progress and a recent-item count. Open the download panel to pause or resume an active transfer, cancel it, reveal a completed file in its operating-system folder, or clear finished entries. The panel opens when a new download starts, floats over the page without resizing it, and keeps completed, cancelled, and interrupted items in memory for the current Hronaut launch. In **Settings → Downloads**, choose a persistent destination, open it in the operating-system file manager, or ask where to save every new website download. Automatic saves remain collision-safe and never overwrite an existing file. Active transfers keep the destination they started with. Pause and resume operate within the current session; resumable interruptions also offer Resume, while terminal failures do not. Servers without range support and matching validators may restart a resumed transfer from zero. The `browser_downloads` MCP tool accepts `list`, `pause`, `resume`, `cancel`, and `clear`; transfer controls require `downloadId` and retain workspace and observation-generation isolation. Returned entries expose `paused` and `canResume` for control availability.
...
Run reachability checks from the environment that actually launches the MCP client. A WSL distribution, container or Dev Container, local VM, Remote SSH session, and hosted runner can each have a different `127.0.0.1` from the Hronaut desktop process. The canonical [client topology guide](https://hronaut.dev/client-topology) separates `app_not_running`, `endpoint_unreachable_from_client_namespace`, `authentication_required`, `mcp_handshake_failed`, and `browser_not_ready`, then verifies the supported native same-host path with health, MCP discovery, and a harmless read of synthetic public content. For deliberate LAN access, enable **Allow connections from other computers** in **Settings → MCP security** and restart Hronaut. It binds all IPv4 interfaces with optional bearer authentication; use the desktop computer’s LAN IP and configured port in the remote client. Direct HTTP is unencrypted and intended only for trusted networks.
...
Tool availability must be checked in the active client or custom-agent context. A valid configuration file and a direct protocol probe can still coexist with a host policy that omits MCP tools. This verification remains entirely local: Hronaut defaults to loopback and requires an explicit Settings opt-in for LAN listeners and does not inspect or control another vendor's client.
...
- `HRONAUT_DISABLE_MCP_AUTH`: force the current launch to keep MCP authentication disabled even if the profile normally requires it. The setting can be changed immediately under **Settings → MCP security**. This override does not change the remote-access setting. Anyone who can reach the listener can control the profile while authentication is off; enable authentication on computers where local processes are not equally trusted.
...
`browser_snapshot` preserves its text content and also returns MCP `structuredContent` with `formatVersion`, `text`, `truncated`, `maxChars`, `returnedChars`, and `omitted` flags for headings, controls, body text, and the overall character budget. Counts use JavaScript UTF-16 string units. `truncated: false` means this bounded snapshot representation was not clipped; it does not claim to cover the entire DOM, hidden content, or every possible control. The existing 80-heading, 500-control, and 300-character label limits also set omission flags. Metadata does not include omitted text or live form values. `browser_find` returns the same metadata without text as `sourceSnapshot`, using its 100,000-character source budget; its existing top-level `truncated` flag still means additional search matches were omitted. An empty search result from a clipped source does not prove the text is absent from the page. Copy Snapshot uses the same measured truncation signal for its clipboard result.

Before using observed content to support a task conclusion, call `browser_snapshot` with `action: "assess-quality"`. It returns a provider-neutral `status` (`candidate`, `empty_content`, `login_wall`, `challenge`, `soft_404`, `wrong_origin`, `needs_review`, or `unknown`), a `continue`, `stop`, or `review` decision, the sanitized resolved URL and origin, content type, bounded document-shape counts, evidence class, and concise reason. Optional `expectedOrigin`, `expectedText`, and `expectedSelector` inputs make task evidence explicit; text and selectors are checked privately and never returned. Every supplied marker must match before it can produce `candidate`, and a challenge, login wall, soft 404, or wrong origin takes priority over marker evidence. Without a marker, `candidate` requires a meaningful rendered primary-content region and heading structure. A candidate result says that the bounded observation is suitable for reasoning; it does not establish account identity, workspace authority, permission to act, or an external postcondition. Stop or request review for every other decision, and assess again after navigation or human intervention.

For a claim that an external result is publicly visible, create a separate clean scratch workspace with `contextClass: "public-observer"` and its exact `observerOrigin`, then call `browser_public_outcome` with that observer workspace and tab plus a distinct standard writer workspace and tab. Both tabs must show the exact credential-free `targetUrl`, and at least one private `expectedText` or `expectedSelector` marker is required. Hronaut observes both contexts concurrently and returns `writer_context_verified` separately from `publicly_observed`, `not_publicly_observed`, `unknown`, or `reconciliation_required`. The receipt includes one observation time, generation-based freshness, and a connection-keyed opaque target fingerprint. It excludes the raw target URL, expected markers, page content, credentials, cookies, customer data, and account identifiers; the keyed fingerprint is intentionally not stable across MCP connections. Both workspaces and origins must already fit the caller's capability profile; the tool cannot widen either scope. The observer stays input-locked, the operation is read-only, and `automaticRetryAllowed` is always false. A missing, delayed, removed, challenged, logged-in, changed, or contradictory observer result must be reconciled or deliberately observed later; it never justifies repeating the original write.

For repeated inspection, call `browser_snapshot` with `action: "set-baseline"` and an optional `maxChars` budget. The response includes the normal bounded snapshot plus an opaque, volatile `baselineId`. Later calls with `action: "delta"`, that ID, and an optional `maxOutputChars` budget return deterministic added, removed, and updated semantic lines, or `status: "unchanged"` without repeating the snapshot. `changeCounts` always describes the complete bounded comparison; `returnedChanges` and `truncated` say how much fit in the requested output. A complete delta advances its baseline by default, while a truncated delta leaves it unchanged so the caller can request a larger budget. Set `advanceBaseline: false` to compare repeatedly with the same state, or use `action: "clear-baseline"` when finished. Navigation, tab or workspace drift, direct human input, reconnect, or other workspace-control changes return `status: "invalidated"` with a stable reason and require a fresh baseline. Baselines live only in bounded process memory, are discarded with their tab, contain the same sanitized snapshot text, do not grant authority, and are not authoritative read-back for an external side effect.

If pause state or workspace authorization changes while a workspace tool is running, its obsolete result is discarded. The error response carries structured `status: STALE_OBSERVATION` for read-only tools or `status: OUTCOME_UNKNOWN` for tools that may write, plus `retrySafe: false` and a next action. Obtain fresh state and make a new decision; a possible side effect is not rolled back or automatically repeated. Target membership is rechecked at settlement except when closing a tab intentionally retires that target. Console, Network, and download evidence carries an opaque `observationGeneration`; pause, disconnect, and workspace resume advance that generation. Diagnostic reads, waits, details, replay, search, debug reports, HAR exports, and workspace download controls exclude older generations. Completed downloads remain visible across later handoffs, while a download that was still in progress or began from a retired request chain must be reviewed in the local Downloads panel. Audit receipt state records the same generation. A delayed redirect can still change the visible page, so inspect the current URL and page after handoff, but its retired request chain cannot become fresh diagnostic evidence.

The preferred interaction loop is `browser_snapshot` → semantic element ref (`e1`, `e2`, ...) → `browser_element_inspect`, `browser_generate_locator`, `browser_click`, or `browser_type`. Use `browser_wait` with a ref or selector when a later action depends on an element becoming attached, detached, visible, or hidden; target waits default to `visible` and cannot be combined with text conditions. Use its `urlPattern` condition for a full URL wildcard such as `*://app.example.com/orders/*` when a redirect or single-page-app route must reach a known destination; it observes both document and same-document navigation and returns the matched URL with credentials, fragments, and recognized secret-bearing query values removed. For canvas, WebGL, maps, remote desktops, and other visual-only surfaces that do not expose a useful DOM target, `browser_click` and `browser_hover` accept viewport-relative CSS-pixel `x` and `y` coordinates, while `browser_drag` accepts `startX`, `startY`, `endX`, and `endY`; every coordinate must fit inside the visible viewport, and coordinate input cannot be combined with element refs or selectors. Coordinate hover and drag use Chromium's native mouse input path, so they can reveal visual states and manipulate pixel-precise targets without executing page-authored JavaScript. Set `doubleClick: true` on `browser_click` for controls such as editable grid cells, file rows, and tree items that require Chromium's native two-click and `dblclick` event sequence. Double-click cannot be combined with dialog handling; use a single click with `dialogAction` for controls that open JavaScript dialogs. When the target text is already known, use `browser_find` to search the same bounded sanitized snapshot and return only compact matching snippets and refs; this avoids transferring the full page context. Take a fresh snapshot after navigation or large DOM changes. Snapshots exclude live form values and sanitize URL credentials, fragments, and recognized secret-bearing query values; page-authored headings, labels, and visible text remain available because they are the interaction context.

Use `browser_press` for named keys, printable characters, and modifier combinations such as `Enter`, `ArrowLeft`, `x`, `Control+A`, or `Control+Shift+R`. Supported modifier names are `Control`/`Ctrl`, `Shift`, `Alt`, and `Meta`/`Command`/`Cmd`. A combination must contain exactly one non-modifier key; use `Control++` when the key itself is plus.

Use `browser_wait` without a text condition to wait for the current page load, with `text` to wait for rendered text to appear, or with `textGone` to wait for rendered text to disappear. `text` accepts either one string or up to 20 candidate strings and resolves with the first candidate found, which is useful when a workflow can finish in one of several success or error states. `text` and `textGone` are mutually exclusive. Text waits poll the rendered `innerText`, stop promptly when the tab closes, default to 30 seconds, and accept an explicit timeout up to 60 seconds. They return only the matched/requested text and outcome rather than a fresh page snapshot.

Every MCP browser workflow creates a fresh workspace with `browser_workspaces` **create** before using page tools. An agent can first call **list** to find sources marked `forkOnly`. It must pass only the UUIDv7 `id` returned by its own create call as `workspaceId` to page tools. Create also returns a private `resumeKey`. A live MCP connection is authorized only for workspaces it created, resumed with that key, or reopened from one of its authorized archives. The normal list returns full state only for those authorized active workspaces; sources whose direct agent access is disabled appear as metadata-only `forkOnly` entries. These entries cannot be browsed, resumed, or changed. The separate `list-fork-sources` action exposes bounded metadata (`id`, `name`, `description`, `color`, `archived`, `agentAccess`) for every active and archived source. Neither list reveals source tabs, URLs, origin inventory, storage IDs, or resume keys for unauthorized sources. When the same task reconnects or Hronaut restarts, it must retain the workspace ID and resume key outside website content and call `browser_workspaces` **resume** before page tools. Archived workspaces use the parallel `browser_saved_workspaces` **resume** action before open or delete. Archive/reopen preserves both the stable workspace ID and resume key, including when the human archives or restores the workspace through Hronaut chrome. Names are only human labels, may repeat across isolated clients, and can change; neither a name nor knowledge of another workspace UUID grants access.

New tabs, bookmark/history opens, website popups, duplicated tabs, closed-tab restores, the implicit active tab, and returned browser state all stay inside the supplied agent workspace. Direct commands, page navigation, redirects, and popups cannot open local files, Hronaut application pages, internal browser pages, other privileged/custom schemes, or URLs with embedded HTTP credentials. Normal credential-free HTTP(S), web-page source views, embedded `data:` QA documents, origin-bound HTTP(S) `blob:` documents, and inert `about:blank` documents remain supported. The separate `browser_file_upload` capability remains an explicit way to attach an agent-supplied absolute local path to a website file input. Hronaut rejects a tab ID from a different workspace and preserves workspaces and their tabs across restarts. Global human New tab commands (including Ctrl/Cmd+T) use the focused tab's workspace, otherwise the first active workspace, and create a normally named workspace only when none exists. The trailing **New tab** button inside a workspace explicitly creates its tab in that workspace.

Click a named workspace header to collapse or expand its tabs, or focus the header and press Enter or Space. Hronaut remembers collapsed workspaces across restarts, keeps the current workspace visibly emphasized, and shows its tab count even while the individual tabs are hidden. Right-clicking a workspace header opens its controls. Tabs can be reordered only inside their current workspace and cannot be moved between workspaces. The workspace editor offers nine labeled colors; agents receive the same palette through `browser_workspaces` create and update operations.

Use **Archive Workspace** from that context menu when an investigation should leave the active tab strip without being lost. Archived workspaces retain their browser profile and reopen their ordered URLs and pinned state under the same stable `workspaceId`. Permanently closing or deleting a workspace deletes its browser data; it does not recreate Default. `browser_saved_workspaces` lists only archives authorized for the current MCP connection, and a reconnect must resume its own archive with the private key returned at creation or save time before opening or deleting it.
...
When a manual step is unavoidable—such as entering credentials, completing a CAPTCHA, approving a device prompt, or pressing a control that cannot be automated—an agent can call `browser_request_user_attention` with a short reason and optional tab ID. The optional `notificationMessage` sets the system notification text; it defaults to the reason. Hronaut marks the requested tab, plays the selected attention cue, pulses its tray icon, and flashes its taskbar entry until the window is focused or the alert is dismissed from the tray menu. The cue can be selected, previewed, or disabled in Settings → Appearance. The pending request is also reported by `browser_status`. Do not include passwords, tokens, or other secrets in either text because the operating system can display them outside Hronaut.

For a durable decision, `browser_human_waiting` provides `request`, `list`, and `cancel` actions scoped to an owned, agent-accessible workspace. A request supplies a UUID `runId`, one of `review-page`, `approve-action`, `provide-input`, or `resolve-unknown`, and optional `owner`, `fallbackOwner`, and `timeoutMs` (1 millisecond to one day; default 15 minutes). Owner labels are at most 128 characters and identify responsibility; they are not authentication credentials. No page text or arbitrary prompt is accepted. A request establishes the workspace continuity guard before recording the decision and requesting local attention.

An `approve-action` request must include one `review` object. It can describe one mutating workspace tool with `toolName`, proposed `arguments`, `reversibility`, and either a bounded `description` or `visible-browser-only` representation, with an optional bounded `expectedPostcondition`. It can instead contain `steps` with two to four ordered `browser_click` reviews. Every grouped step requires both the click's declarative `postcondition` and a bounded human-readable `expectedPostcondition`; this keeps advancement tied to authoritative read-back rather than transport success. Hronaut validates and normalizes every proposed argument object with the target tool schema, then retains only domain-separated SHA-256 artifact hashes and bounded review metadata. Raw selectors, entered values, scripts, credentials, and other proposed arguments are not written to human-decision history. The visible review identifies every ordered step plus the workspace, capability profile, current safe origin and tab, action class, reversibility, state generations, deadline, exact artifact hashes, and bounded receipt timeline.

Records have an ID and revision, deadline, notification status and attempt count, prior-outcome classification, and a distinct `WAITING_FOR_HUMAN`, `ACKNOWLEDGED`, `RESOLVED`, `REJECTED`, `EXPIRED`, `CANCELLED`, `ATTEMPTED`, `VERIFIED`, or `UNKNOWN` state. Listing needs no live tab and remains available after reconnect once workspace ownership is resumed. Cancellation requires the exact ID and revision. Records are stored locally with bounded capacity; unresolved records are never evicted to admit another request. A storage failure is reported instead of an empty history. After restart an unexpired ordinary decision requires fresh acknowledgement; proposed and approved consequential reviews expire, and an attempted action becomes unknown.

Open **Edit workspace → Human decisions** to refresh decisions, acknowledge receipt, reject or cancel a proposed action, or complete a review. Grouped reviews show every ordered action and postcondition before approval. Completion requires an explicit checkbox and a fresh workspace continuity review above the panel, checked again after saving. Agents cannot acknowledge, reject, or complete human decisions through MCP. Pending decisions block consequential page tools while inspection remains available. An approved consequential review also blocks every unrelated mutation. Its `id` and current `revision` can be supplied once to the exact normalized tool call; a changed artifact, origin, tab, navigation generation, human-input generation, workspace, scope, session, expiry, or continuity state invalidates it.

Before that exact tool dispatch, Hronaut durably records `ATTEMPTED`. A matching postcondition changes a single action to `VERIFIED`. For a group it advances to the next exact step, issues a fresh revision in `reviewContinuation`, and keeps unrelated mutations blocked; only the last verified step completes the group. An error or ambiguous result changes the review to `UNKNOWN`; a successful single action without authoritative postcondition remains `ATTEMPTED`. Restart converts an unresolved attempt to `UNKNOWN` and expires proposed or approved handles, so reconnecting cannot replay an old approval. Acknowledgement, rejection, cancellation, expiry, and completion never replay actions, unpause agents, erase an unknown outcome, or automatically clear the separate continuity guard. Notification delivery describes dispatch to local attention controls, not proof that a person saw or understood it.
...
`browser_accessibility_audit` runs the bundled axe-core engine locally inside one website tab and returns prioritized WCAG violations without uploading the page to an audit service. It checks WCAG A/AA by default, can target one CSS selector, and can instead include AAA, best-practice, or all enabled rules. Agent responses are bounded to 50 rules and 10 affected nodes per rule; they contain selectors and failure summaries, not element HTML or form values. Humans can run the same A/AA audit from Page tools, review critical and serious findings, open rule guidance, and rerun after a fix.

Use `action: "set-baseline"` before a change and the default `measure` action afterward to compare stable rule-and-target fingerprints. The Page tools panel offers the same Save, Replace, Measure, and Clear workflow and lists new, remaining, and resolved findings. The baseline stays in memory for one tab, intentionally survives navigation, and disappears when cleared, when the tab closes, or when Hronaut exits. Comparisons flag URL, selector, standard, collection-limit, and axe-engine changes. A changed selector, standard, collection limit, or engine makes the result non-comparable. If the baseline was truncated, new findings remain unknown; if the current audit was truncated, resolved findings remain unknown. This prevents omitted results from being presented as regressions or fixes. Automated checks cannot prove complete accessibility, so keyboard and assistive-technology testing remain necessary.
...
`browser_dialog` accepts or dismisses an already-open JavaScript `alert` or `confirm` dialog. An agent that knows a click or evaluation will open one can instead provide `dialogAction` directly to `browser_click` or `browser_evaluate`. For a website click that opens `window.prompt()`, `browser_click` also accepts `promptText` together with `dialogAction: "accept"`; invalid or dismissing prompt-text combinations fail before clicking. Hronaut responds inside the same operation so the page cannot deadlock the tool call. Electron does not implement native prompts, so Hronaut installs a prompt response only around that explicit click; all other dialogs remain native and user-visible.

`browser_webmcp` makes Hronaut an MCP-to-WebMCP consumer bridge; it does not make Hronaut's local MCP server available to page JavaScript and does not add WebMCP to a website. The tool is available in Web QA and Complete profiles, but not Browser Essentials. `status` feature-detects the browser-native `document.modelContext` API, `list` returns bounded tools registered by the selected top-level HTTP(S) document, and `call` requires the exact workspace tab, origin, navigation generation, descriptor digest, name, and arguments from that listing. Same-origin iframe tools are deliberately excluded. A call re-lists the tools immediately before dispatch, rejects descriptor or schema drift, treats page descriptions, annotations, errors, and results as untrusted content, and remains classified as open-world and potentially destructive even when the page labels a tool read-only. Its returned value and fresh bounded page snapshot separate dispatch from observation; neither proves that an external side effect succeeded. Hronaut enables Electron's native experimental WebMCP implementation without injecting a polyfill, and an explicit Chromium `--disable-features=WebMCP` policy keeps it unavailable.
...
- MCP authentication is off for new profiles and remote access defaults off. Remote access takes effect after restart and listens on all IPv4 interfaces. Authentication is independently configurable and changes immediately. Without authentication, anyone who can reach the listener can control the profile. Resetting MCP settings disables authentication immediately and remote access after restart. Enable **Require MCP authentication** in **Settings → MCP security** when local processes are not equally trusted; Hronaut then generates and reuses an owner-only token unless a valid `HRONAUT_MCP_TOKEN` override is supplied.
...
Available since Hronaut 2.4.21, the local-wallet preview supports EVM, Solana, and Tron. Preview means the provider and automation surface may evolve; the feature is included in published builds. Hronaut can generate or import managed wallets, add watch-only addresses, and create dedicated agent wallets. Managed signing material stays in the trusted Electron main process inside an authenticated encrypted vault; websites and MCP clients refer to public wallet IDs and can request scoped access, simulation, signing, submission, status, or cancellation without receiving private keys or recovery phrases.

Wallet access is scoped to the wallet, workspace, top-level origin, account, network, requester, and capability. Mainnet defaults to explicit human approval in trusted Hronaut chrome. A dedicated EVM agent wallet may use a user-created, short-lived **Bypass Approve** policy that binds one exact workspace, network, origin, destination, and method with complete amount, spend, fee, simulation, operation-count, and expiry limits. Websites and all requests outside that delegation still require a person. Other bounded automatic approval is limited to allowlisted EVM testnet chain IDs or known loopback local chains; configured, requested, and RPC-reported chain IDs must agree, but this is not independent proof of endpoint honesty. It fails closed for undecodable requests, unsuccessful simulation, policy-limit violations, or changed approval data. Hronaut does not integrate WalletConnect, Reown, or external-wallet SDKs in this iteration. See the [wallet security and usage reference](docs/WALLETS.md) for supported provider methods, onboarding and recovery, Linux secret-store behavior, policy limits, RPC configuration, revocation, testing, and the exact threat boundary. The [watch-only public-address wallet QA recipe](docs/WATCH_ONLY_WALLET_QA.md) verifies read-only observation without importing signing material; the separate [local EVM wallet QA quickstart](docs/WALLET_QA_QUICKSTART.md) provides a disposable signing first run with independent receipt readback. The [external-wallet and mobile dApp QA boundary](docs/EXTERNAL_WALLET_QA_BOUNDARIES.md) separates those Hronaut-local checks from extension, WalletConnect, proprietary SDK, and mobile-wallet suites.
...
The CI workflow runs lint, unit tests, the production build, the Playwright Electron suite, a raw Electron dialog test without a competing automation debugger, and the production dependency audit. Playwright retains traces from failed attempts. The Electron fixtures begin recording after the first window finishes DOM initialization so snapshot installation cannot race Electron startup. They explicitly record page snapshots and screenshots for their browser contexts, including manually launched applications and instances closed before a later test failure. The recorder honors file-level trace overrides. Intentional renderer-crash cases disable frame recording to avoid a late-response assertion in Playwright's CDP driver, while retaining DOM snapshots and call/network evidence. Passing tests discard these temporary archives. A closed process or an export that exceeds the five-second cleanup deadline produces bounded diagnostic metadata without preventing application shutdown. CI permits one diagnostic retry but fails on flaky tests, so a retry that passes cannot turn the gate green; local non-CI Playwright runs do not retry. The Docker runners set CI mode and use the same strict retry gate. On an integration failure, CI copies the traces, test output, and self-contained HTML report out of the stopped Docker container through a runner-owned tar stream, removes the container, and keeps the uploaded diagnostics for seven days. Tagged releases repeat lint, unit, build, dependency-audit, and the same failure-diagnostics validation against the immutable tag before any platform package build starts. Pull requests whose branch name starts with `release/` additionally build the complete Linux, macOS, and Windows package sets in a hosted matrix before merge.
...
The release workflow creates one draft release, builds Linux, macOS, and Windows artifacts in parallel from the immutable tag commit, uploads the updater manifests and a `hashes.txt` SHA-256 manifest, and publishes only after every platform job succeeds. Uploads are reconciled one at a time against the draft by filename, uploaded state, size, and GitHub's SHA-256 digest. A failed response is read back before any replay, verified assets are preserved, transient retries are bounded, and invalid requests or authorization failures stop without retrying. If the attempt budget is exhausted, fix any reported nonretryable error and rerun the failed release job: the same draft is the resume point and already verified assets are skipped. The workflow refuses conflicting, unexpected, or already-public assets rather than replacing them. Packaged Hronaut builds use those manifests through `electron-updater`. After public download routes are verified, the workflow updates the matching Scoop manifest from the published Windows portable checksum and dispatches full CI plus the Windows Scoop install/reinstall smoke. It skips that write if `main` has already advanced to another package version. The workflow can also be dispatched manually for an existing version tag.
