...
Every MCP browser workflow starts with `browser_workspaces` **create**. The agent must create a fresh workspace for its task and pass only the UUIDv7 `id` returned by that call as `workspaceId` to every other browser tool. Create also returns a private `resumeKey`. A live MCP connection is authorized only for workspaces it created, resumed with that key, or reopened from one of its authorized archives; owned-workspace list operations omit the human **Default** workspace, every other client's workspaces and archives, and workspaces whose direct agent access the human has disabled. The separate `list-fork-sources` action intentionally exposes bounded metadata (`id`, `name`, `color`, `archived`, `agentAccess`) for every active and archived source, including sources with direct agent access disabled; it reveals no tabs, URLs, origin inventory, storage IDs, or resume keys and grants no direct access. When the same task reconnects or Hronaut restarts, it must retain the workspace ID and resume key outside website content and call `browser_workspaces` **resume** before page tools. Archived workspaces use the parallel `browser_saved_workspaces` **resume** action before open or delete. Archive/reopen preserves both the stable workspace ID and resume key, including when the human archives or restores the workspace through Hronaut chrome. Names are only human labels, may repeat across isolated clients, and can change; neither a name nor knowledge of another workspace UUID grants access.
