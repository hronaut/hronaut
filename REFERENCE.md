...
Records have an ID and revision, deadline, notification status and attempt count, prior-outcome classification, and a distinct `WAITING_FOR_HUMAN`, `ACKNOWLEDGED`, `RESOLVED`, `EXPIRED`, or `CANCELLED` state. Listing needs no live tab and remains available after reconnect once workspace ownership is resumed. Cancellation requires the exact ID and revision. Records are stored locally with bounded capacity; unresolved records are never evicted to admit another request. A storage failure is reported instead of an empty history. After restart an unexpired unresolved decision requires fresh acknowledgement; elapsed deadlines remain expired.

Open **Edit workspace → Human decisions** to refresh decisions, acknowledge receipt, or cancel a decision. Completion requires an explicit checkbox and a fresh workspace continuity review above the panel, checked again after saving. Agents cannot acknowledge or complete human decisions through MCP. Pending decisions block consequential page tools while inspection remains available. Acknowledgement, cancellation, expiry, and completion never replay actions, unpause agents, erase an unknown outcome, or automatically clear the separate continuity guard. Notification delivery describes dispatch to local attention controls, not proof that a person saw or understood it.
