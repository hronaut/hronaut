...
See [docs/WALLETS.md](docs/WALLETS.md) for the threat model, supported methods, Linux vault behavior, recovery, RPC configuration, revocation, and the exact security boundary. Use the [local EVM wallet QA quickstart](docs/WALLET_QA_QUICKSTART.md) for a disposable Anvil first run with receipt reconciliation. Hronaut does not use WalletConnect, Reown, hosted custody, or proprietary wallet SDKs.
